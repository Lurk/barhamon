...
export const nastiest_bugs: PostInterface = {
  ...
  full: [
    preview,
    p("Oh boy. What to choose? Throughout my career, I dealt with many tricky bugs, one tricker than the other. But two of them stand out. So buckle up. Here is tail about the nastiest bugs in my life."),
    h3("Bug with blurry images."),
    p("Once upon a time, I did a UI framework for IPTV set-top boxes. It was React + Redux app, on top of a Chromium custom build, on top of 4.4.2 Android."),
    p("One day, I got a bug report - Channel icons are somewhat blurry. And two weeks of fun begins."),
    p("First of all, I checked the obvious things. Like maybe images are blurry in the first place? Or perhaps somehow I screwup the viewport in Head. Or whatever. But no, everything was fine except blurry images."),
    p("And finally, after a couple of weeks of reading documentation, make files, and source code of Chromium, I found out that there was a \"feature\" in Chromium with pixel density. The browser was detecting the Full HD screen on the Android device, which means we have a high dpi screen, but we do not. So every image, before rendering, first gets scaled up and then scaled down by the browser."),
    p("The only reasonable solution was to hardcode scale. So I patched our version of Chromium to use the constant scale."),
    h3("The server is on fire, customers on fire, me, also on fire. TCP.LAST_AKK timeout 60 seconds."),
    ...
    createMessage(
      { content: p("The patch, which enables control of LAST_AKK timeout by finwait2_timeout, sits in the FreeBSD tracker since") }
    ),
    ...
    createMessage({
        content:
          p([
            ...
            a({ url: "https://calomel.org/freebsd_network_tuning.html", text: "best article about performance tuning" }),
            " I ever read."
          ])
      }
    ),
    p("Honestly, I still don't know why exactly it has happened. But the fix and the lesson was - do not use HTTP for local services, use Unix sockets instead.")
  ],
}
