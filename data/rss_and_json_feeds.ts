...
export const rss_and_json_feeds_for_next_js_site: PostInterface = {
  ...
  full: [
    ...
    p("Newerless RSS feeds are widely supported, so this blog should provide a new and shiny JSON feed and an old and dusty RSS."),
    p([
      "If you read the ",
      a({
    ...
    p([
      "One significant difference is memoization. Since posts are immutable, we can remember the result of feed generation. I wrote a simple helper function. It is not a general-purpose memoization function because it will only work with a one-argument function and make only shallow comparisons. This is intentional and done for simplicity's sake. You can find the source code here ",
      a({
        text: "utils/index.ts",
