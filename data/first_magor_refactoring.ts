...
export const first_major_refactoring: PostInterface = {
  ...
  full: [
    ...
    p("Now posts are stored not as React components but as a serializable tree."),
    p("This refactoring will allow not only to separate the \"engine\" from the data but also to separate the templates into a separate package. That means that sooner or later, there will be more than one theme :)"),
    p("And the data itself can now be stored anywhere, which means that there will be connectors to MongoDB, Postgres, or JSON files."),
